...
interface IFilestackLogoProps {
  className?: string;
  /** When true, render only the icon (no wordmark). */
  iconOnly?: boolean;
  /** When true, render the wordmark in white instead of the default. */
  invert?: boolean;
}

/**
 * Official Filestack logo (sourced from filestack.com).
 *
 * The original SVG combines the folded-document icon + "filestack" wordmark
 * in a single 121×32 viewBox. The icon path is the first <path> (orange);
 * the wordmark is the second group (dark text).
 */
export const FilestackLogo: FC<IFilestackLogoProps> = ({ className, iconOnly, invert }) => {
  if (iconOnly) {
    return (
      <svg
        viewBox="0 0 28 32"
        xmlns="http://www.w3.org/2000/svg"
        aria-label="Filestack"
        className={cn("h-5 w-5 shrink-0", className)}
      >
        <path
          d="M25 22V3H3v26h15v3h10V22zM0 0h28v32H0zm20 24h8l-8 8zM9 9h10v3H9zm0 5h8v3H9zm0 5h3v3H9z"
          fill="#ef4a25"
        />
      </svg>
    );
...
interface IFilestackBadgeProps {
  variant?: "light" | "dark";
  className?: string;
}

export const FilestackBadge: FC<IFilestackBadgeProps> = ({ variant = "light", className }) => {
  ...
  return (
    <a
      ...
      className={cn(
        "inline-flex items-center gap-3 rounded-full px-4 py-2 transition-all",
        isDark
          ? "bg-white/10 text-white hover:bg-white/20"
          : "border border-slate-200 bg-white text-slate-900 hover:border-orange-300 hover:shadow-sm",
        className,
      )}
    >
      <span className="text-[10px] font-bold uppercase tracking-widest opacity-60">Powered by</span>
      <FilestackLogo invert={isDark} />
    </a>
  );
};
